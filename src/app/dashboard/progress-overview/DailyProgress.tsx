  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded-lg w-1/3">
        <h2 className="text-xl font-bold text-gray-900">Set Target for {targetTitle}</h2>
        <div className="mt-4">
          <input
            type="number"
...
const DailyProgress = () => {
  ...
  const handleModalSubmit = (newTarget) => {
    if (currentItem) {
      const updatedProgressData = [...progressData]; // Copy the current data
      const updatedItem = updatedProgressData.find(
        (item) => item.title === currentItem.title
      );
      if (updatedItem) {
        updatedItem.target = parseInt(newTarget, 10); // Update the target of the selected item
        updatedItem.percentage = (updatedItem.current / updatedItem.target) * 100; // Recalculate percentage based on the new target
      }
      setProgressData(updatedProgressData); // Update the progress data state
    }
  };
